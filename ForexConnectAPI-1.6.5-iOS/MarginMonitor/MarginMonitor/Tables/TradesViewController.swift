...
class TradesTableRow : Comparable
{
    ...
    public let mmr: Double
    
    public let parsedPL: String
    public let parsedAmount: String
    public let parsedMmr: String
    
    private let numberByOrder: Int64

    init(tradesId: String, instrument: String, buySell: String, amount: Int, pl: Double, mmr: Double, isTotalRow: Bool) {
        ...
        if isTotalRow {
            ...
            self.parsedMmr = mmr.parseToPlaces(places: 2)
        }
    }
...
class TradesTable : IO2GTableListener, IO2GEachRowListener
{
    ...
    private func addTrade(row: IO2GTradeTableRow) {
        ...
        let pl = row.getPL()
        let mmr = row.getUsedMargin()
        let tradesId = row.getTradeID()
    
        let tableRow: TradesTableRow = TradesTableRow(tradesId: tradesId!, instrument: instrument!, buySell: buySell!, amount: amount, pl: pl, mmr: mmr, isTotalRow: false)
    ...
    func calcTotalAmount(instrument: String) -> Int {
        var totalAmount = 0
        for (_, tradeRow) in tradesTableRows {
            if tradeRow.instrument != instrument { continue }
            totalAmount += ( tradeRow.buySell == "B" ? tradeRow.amount : -tradeRow.amount)
        }
        return totalAmount
    }
...
class TradesViewController : UITableViewController
{
    ...
    override func viewDidLoad() {
        ...
        headerLabel.text = "    Symbol     Amount       S/B       P/L      MMR  "
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let mmr = tradesTableRows[indexPath.row].parsedMmr
    ...
    func tradesUpdated(tradesTableRows: [TradesTableRow]) {
        ...
        DispatchQueue.main.async {
            // calc total PL
            var summPl = 0.0, sumMMR = 0.0
            for tradeRow in tradesTableRows {
                summPl += tradeRow.pl
            }

            // add total row
            let totalRow = TradesTableRow(tradesId: "", instrument: "Total", buySell: "", amount: 0, pl: summPl, mmr: 0, isTotalRow: true)
