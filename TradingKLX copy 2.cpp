...
#include "PerformanceAssessor.h"  // Include the Performance Assessor
...
// Process and rescale the market data before calculating indicators
void processMarketData(std::vector<PriceData>& priceData, std::vector<double>& highs, 
                       std::vector<double>& lows, std::vector<double>& closes) {
    for (auto& data : priceData) {
        // Rescale prices to bring them into realistic range for EUR/USD (e.g., 1.xxxxx)
        data.high *= 100000;
        data.low *= 100000;
        data.close *= 100000;

        // Cap Close prices at a realistic maximum (e.g., 1.5 for EUR/USD)
        data.close = std::min(data.close, 1.5);
...
void logSignals(const std::vector<TradingSignal>& signals) {
    for (const auto& signal : signals) {
        if (signal.buy) {
            ...
            std::cout << "Sell signal at index: " << signal.index << ", Profit: " << signal.profit << std::endl;  // Log the profit
        }
    }
}
...
int main() {
    ...
    // Initialize new risk settings in the strategy class: $10,000 starting balance, 0.15% risk, stop-loss multiplier 1.25
    ...
    // Create threads for parallel execution of Ichimoku and Bollinger Bands calculation
    ...
    // Evaluate buy/sell signals based on SMA, position size, Ichimoku, and Bollinger Bands
    ...
    // Track portfolio values and assess performance
    ...
    for (const auto& signal : signals) {
        currentBalance = strategy.getAccountBalance();  // Get balance after each trade
        portfolioValues.push_back(currentBalance);
    }

    // Use PerformanceAssessor to evaluate the backtest results
